...
var dateFormat = function dateFormat() {
  ...
  formatter = formatter.replace('YYYY', year).replace('MM', String(month)[1] ? month : "0".concat(month)).replace('DD', String(day)[1] ? day : "0".concat(day)).replace('hh', String(hour)[1] ? hour : "0".concat(hour)).replace('mm', String(minute)[1] ? minute : "0".concat(minute)).replace('ss', String(second)[1] ? second : "0".concat(second)).replace('SSS', millisecond).replace('ww', weekDay).replace('WW', weekText + weekArray[weekDay]);
  return formatter;
};
